      ...
      <BRow class="mb-3">
        <BFormGroup label="取引先名" label-for="clientName" label-cols="3">
          <BFormInput
            ...
            :state="form.clientName.length > 0"
            maxlength="20"
            required
          />
        </BFormGroup>
...
// 編集画面かどうか
const isEdit = computed(() => route.params.clientCode != null);
...
onMounted(async () => {
  ...
  if (isEdit.value) {
    ...
    loading.value = true;
    try {
      const clientDetail = await clientApi.getClientByClientCode(route.params.clientCode);

...
describe("clientService", () => {
  ...
  describe("findByCode 取引先情報詳細取得", () => {
    ...
    test("[異常系] 存在しない取引先コードを指定した場合 => NotFoundErrorとなること", async () => {
      ...
      // テスト対象関数の呼び出しと検証
      await expect(clientService.findByCode(clientCode)).rejects.toThrow(NotFoundError); // スローしたエラーの検証
      expect(spy).toHaveBeenCalledTimes(1); // Mockした関数の呼び出し回数を検証
      expect(spy).toHaveBeenCalledWith(clientCode); // Mockした関数呼び出し時の引数を検証
    });
  });
  ...
  describe("create 取引先情報登録", () => {
    test("[正常系] 存在しない取引先コードを指定した場合 => 正常終了すること", async () => {
      ...
      // 検証
      expect(spyFindByCode).toHaveBeenCalledTimes(1); // Mockした関数[findByCode]の呼び出し回数を検証
      expect(spyFindByCode).toHaveBeenCalledWith(clientCode); // Mockした関数[findByCode]呼び出し時の引数を検証
      expect(spyCreate).toHaveBeenCalledTimes(1); // Mockした関数[create]の呼び出し回数を検証
      expect(spyCreate).toHaveBeenCalledWith(clientInfo); // Mockした関数[create]呼び出し時の引数を検証
    });
    ...
    test("[異常系] 存在する取引先コードを指定した場合 => UniqueConstraintErrorとなること", async () => {
      ...
      // テスト対象関数の呼び出しと検証
      await expect(clientService.create(clientInfo)).rejects.toThrow(UniqueConstraintError); // スローしたエラーの検証
      expect(spyFindByCode).toHaveBeenCalledTimes(1); // Mockした関数[findByCode]の呼び出し回数を検証
      expect(spyFindByCode).toHaveBeenCalledWith(clientCode); // Mockした関数[findByCode]呼び出し時の引数を検証
  ...
  describe("update 取引先情報更新", () => {
    test("[正常系] 存在する取引先コードを指定した場合 => 正常終了すること", async () => {
      ...
      // DBに存在するデータ
      const client = { clientCode: "a0000001", clientName: "A株式会社", orderKbn: "1" };
    ...
    test("[異常系] 存在しない取引先コードを指定した場合 => NotFoundErrorとなること", async () => {
      ...
      // テスト対象関数の呼び出しと検証
      await expect(clientService.update(clientCode, clientInfo)).rejects.toThrow(NotFoundError);
  ...
  describe("delete 取引先情報削除", () => {
    test("[正常系] 取引先情報削除処理を呼びだすこと", async () => {
      // 検索条件
      ...
      // テスト対象関数の呼び出し
      const actual = await clientService.delete(clientCode);
    ...
    test("[異常系] 存在しない取引先コードを指定した場合 => NotFoundErrorとなること", async () => {
      ...
      // テスト対象関数の呼び出しと検証
      await expect(clientService.delete(clientCode)).rejects.toThrow(NotFoundError); // スローしたエラーの検証
      expect(spyFindByCode).toHaveBeenCalledTimes(1); // Mockした関数の呼び出し回数を検証
      expect(spyFindByCode).toHaveBeenCalledWith(clientCode); // Mockした関数呼び出し時の引数を検証
    ...
    test("[異常系] 受発注情報に取引先コードが使用されている場合 => ReferenceConstraintErrorとなること", async () => {
      ...
      // テスト対象関数の呼び出しと検証
      await expect(clientService.delete(clientCode)).rejects.toThrow(ReferenceConstraintError); // スローしたエラーの検証
      expect(spyFindByCode).toHaveBeenCalledTimes(1); // Mockした関数の呼び出し回数を検証
      expect(spyFindAll).toHaveBeenCalledWith({ clientCode: clientCode }); // Mockした関数呼び出し時の引数を検証
      expect(spyFindAll).toHaveBeenCalledTimes(1); // Mockした関数の呼び出し回数を検証
      expect(spyDelete).not.toHaveBeenCalled(); // Mockした関数の呼び出し回数を検証
